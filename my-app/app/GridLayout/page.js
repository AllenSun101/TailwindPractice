...
export default function GridLayout() {
  ...
  return (
    <div className={"" + (dark ? "bg-blue-100" : "bg-red-100")}>
      <div className="text-green-400 text-6xl min-h-screen grid grid-cols-1 sm:grid-cols-3 justify-center items-center">
        <button className="bg-white relative after:absolute after:bg-green-300 after:inset-0 text-slate-900 group p-10
        hover:opacity-50 duration-300 flex items-center gap-4 after:z-0 after:blur sm:col-span-2">
          <h1 className="z-10">BUTTON</h1>
          <span className="z-10 h-10 w-10 bg-amber-800 group-hover:rotate-45 duration-300 hidden sm:inline"></span>
        </button>
        <button className="bg-white relative after:absolute after:bg-green-300 after:inset-0 text-slate-900 group p-10
